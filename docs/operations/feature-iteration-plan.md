...
## 13. 下一轮建议（Iteration 9：Guardian Trust Evidence Loop）

在 Iteration 8 后，瓶颈从“策略可执行”转向“策略可证据化”：
系统已经能根据信号调节强度，但尚缺统一证据结构来稳定回答“为何触发、依据何在、效果如何”。

### 目标
- 为 intervention policy 增加稳定 `policy_version`，确保策略语义可追踪。
- 为 policy 输出增加结构化 `evidence`（信号、响应分布、trust_repair 轨迹），消除黑盒决策感。
- 在 `/state` 投影 policy 证据，支持前端和运维面板直接消费。
- 补齐回归测试，锁定无数据与有数据场景的契约稳定性。

### 已实施范围（2026-02-25）
- `core/retrospective.py`
  - 在 `intervention_policy` 中新增：
    - `policy_version = guardian_policy_v1_evidence_loop`
    - `evidence.window_days`
    - `evidence.active_signals`
    - `evidence.response_events.action_counts/context_counts`
    - `evidence.trust_repair`（active/reason/streak/last_negative_at 等）
- `web/backend/routers/api.py`
  - `/state` 的 `audit.used_state_fields` 新增：
    - `retrospective.intervention_policy.policy_version`
    - `retrospective.intervention_policy.evidence`
  - `guardian` 投影新增：
    - `policy_version`
    - `policy_evidence`
  - `guardian.metrics` 同步新增：
    - `policy_version`
    - `policy_evidence`
- 测试
  - `tests/test_retrospective_signals.py` 新增 policy evidence payload 覆盖。
  - `tests/test_goal_service.py` 更新 `/state` 契约断言，覆盖 `policy_version/policy_evidence`。
  - 目标测试集通过：
    - `tests/test_retrospective_signals.py`
    - `tests/test_goal_service.py`
    - `tests/test_guardian_outcome_loop.py`

### 验收结果
- `intervention_policy` 已具备可追踪版本号与证据结构，且不影响原有字段兼容。
- `/state` 已稳定返回 policy 证据投影，可被上层 UI 与运营逻辑直接消费。
- 回归结果：`78 passed`。

### 风险与后续
- 风险：evidence 字段继续扩展时可能导致消费者耦合上升。
- 控制：保持 `policy_version` 前向兼容策略，新增字段只追加不移除。
- 建议后续：在前端增加 policy evidence 卡片，展示“触发依据 + 最近响应分布 + trust_repair 轨迹”。
