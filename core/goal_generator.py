...
class GoalGenerator:
    ...
    def _build_prompt(self, profile: UserProfile, n: int) -> str:
        ...
        return f"""
        You are an advanced AI Life Coach designed to help the user achieve a HOLISTIC, balanced, and self-actualized life.
        ...
        1. **Career/Growth**: A hard skill or professional achievement related to their occupation. (The "Game Changer")
        2. **Health/Vitality**: A goal focused on physical or mental well-being (e.g., sleep, exercise, diet, meditation).
        3. **Exploration/Joy**: A goal focused on hobbies, creativity, social connection, or pure curiosity (low pressure).
    ...
    def decompose_to_children(self, parent_goal: Goal, profile: UserProfile, n: int = 4, context: dict = None, existing_titles: List[str] = None) -> List[Dict]:
        """
    ...
    def _fallback_questions(self) -> List[Dict]:
        """备选问题"""
        return [
            {"id": "q1", "question": "你目前在这个领域的经验水平？", "options": ["新手", "有一定经验", "专家"]},
            {"id": "q2", "question": "你每周可以投入多少时间？", "options": ["<5小时", "5-10小时", "10-20小时", ">20小时"]}
        ]
    
    def _fallback_options(self, parent_goal: Goal, n: int) -> List[Dict]:
        """LLM 失败时的备选选项"""
        return [
            {"title": f"{parent_goal.title} - 方式 {i+1}", "description": "请自定义描述", "probability": 50}
            for i in range(n)
        ]
