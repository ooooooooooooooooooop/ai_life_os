...
def load_model_config(profile_name: Optional[str] = None) -> Dict[str, Any]:
    ...
    if "profiles" in raw_config:
        ...
        if raw_config.get("force_local_mode", False):
            print(f"[System] [WARN] 强制本地模式 (Force Local Mode) 已激活。")
        ...
        if isinstance(value, str):
            ...
            if match:
                ...
                if env_value:
                    ...
                    raise ValueError(
                        f"配置未从简 (local_model.yaml): '{key}' 字段当前为占位符 '${{{env_var}}}'。\n"
                        f"请打开 config/local_model.yaml，直接将 '${{{env_var}}}' 替换为您真实的 API 密钥字符串。"
                    )
            else:
                result[key] = value
...
def create_llm_adapter(config: Optional[Dict[str, Any]] = None, profile_name: Optional[str] = None) -> BaseLLMAdapter:
    """
    ...
    if provider == "openai":
        ...
        raise ValueError(f"[配置错误] 未知的 LLM provider: '{provider}' (Profile: {profile_name})。请检查 config/model.yaml 或 local_model.yaml。")
