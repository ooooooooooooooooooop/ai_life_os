...
class Blueprint:
    ...
    def evaluate(self, goal: Goal, context: UserProfile, mock_llm_response: Dict = None) -> BlueprintScore:
        """
        评估单个目标。
    ...
    def _evaluate_with_llm(self, goal: Goal, context: UserProfile) -> Dict[str, float]:
        ...
        principles_text = "\n".join([f"- {p['name']}: {p.get('description', '')}" for p in self.principles])
    ...
    def filter_and_rank(self, goals: List[Goal], context: UserProfile) -> List[Tuple[Goal, BlueprintScore]]:
        """
        过滤并排序候选目标。
        返回 [(Goal, Score), ...]
        """
    ...
    def _check_hard_rules(self, goal: Goal, context: UserProfile) -> Tuple[bool, str]:
        ...
        if estimated_hours > daily_limit * 1.5:
            return False, f"Estimated time ({estimated_hours}h) exceeds daily limit ({daily_limit}h * 1.5)"
